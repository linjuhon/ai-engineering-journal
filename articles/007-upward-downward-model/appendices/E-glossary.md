...
| **S 賽道** | 唯一被允許的正確執行路徑，有明確的 entry/exit checkpoint |
| **Route Lock** | 封路機制：鎖定 S 賽道，排除所有其他路徑 |
| **State Lock** | 鎖狀態機制：確保分析過程中系統狀態不被改變 |
| **First Divergence** | 第一個 actual 與 expected 超過 tolerance 的位置 |
| **Repair Permission** | 修復許可：只有通過 Numeric Truth 後才能 patch |
| **Downward Model (D↓)** | 從目的往下生成細節的模型 |
| **Upward Model (U↑)** | 從結果往上反推目的的模型 |
| **Purpose Lock** | 目的鎖定：purpose vector 鎖定後不可擅自修改 |
| **Event Gate Lock** | 事件閘門鎖定：gate 通過事件解鎖下一個 gate |
